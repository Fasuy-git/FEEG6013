...
#figure(
  ...
  caption: [Image depicting the four Denavit-Hartenberg parameters ($theta,a, R, beta$). *@dataset*],
  supplement: [Figure],
  kind: figure,
)<2-DH-image>
...
#figure(
  ...
  caption: [Image depicting the crease lines (shown in red) on a thin panel Miura-ori unit cell meeting at one point [Left] and crease lines not intersecting at one point for a thick panel configuration of the Miura-ori unit cell [Left.] *@offset_geometry*],
  supplement: [Figure],
  kind: figure,
)<2-crease-lines>
...
Applying the conditions in *@2-thick-origami-substitutions* to the set of thick panel equations, it was found that the three equations with $a$ terms appeared as either a common factor on both sides of the equation or formed an expression equal to zero. As a result the $a$ terms could be cancelled out of the closure condition equations. The complete derivation depicting this is contained within *Appendix A*. Therefore, under the geometrical assumptions contained within *@2-thick-origami-substitutions*, the panel thickness did change the configuration of the spatial linkages but had no impact on the kinematic equations. The remaining non-trivial equations simplify in the same manner as the thin origami configuration resulting in the set of equations shown in *@2-kinematic-relationship-equations* *@dataset* which can be used to model the kinematic behaviour of thin or thick panel origami.
